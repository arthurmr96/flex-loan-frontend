...
interface Erc721ContractRead {
  isApprovedForAll(contractAddress: string, owner: string): Promise<boolean>
}

export const erc721ContractRead = (): Erc721ContractRead => {
  return {
    async isApprovedForAll(contractAddress: string, owner: string): Promise<boolean> {
      try {
        ...
        const isApproved = await erc721.isApprovedForAll(owner, contractAddress)
